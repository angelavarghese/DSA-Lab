...
NODE addterm(NODE res, int co, int po) {
    NODE temp, cur;
    if (co == 0) return res; // Skip zero coefficient terms

    if (res == NULL) { // Handle empty result list
        temp = (NODE)malloc(sizeof(struct node));
        temp->co = co;
        temp->po = po;
        temp->next = NULL;
    ...
    cur = res;
    while (cur != NULL) {
        if (cur->po == po) { // Merge terms with the same power
            cur->co += co;
            return res;
        }
        if (cur->next == NULL) break; // Stop at the last node
        cur = cur->next;
    }

    // If no matching power, insert at the end
    temp = (NODE)malloc(sizeof(struct node));
    temp->co = co;
    temp->po = po;
    temp->next = NULL;
...
NODE multiply(NODE poly1, NODE poly2) {
    ...
    for (p1 = poly1; p1 != NULL; p1 = p1->next) {
        for (p2 = poly2; p2 != NULL; p2 = p2->next) { // Corrected condition
